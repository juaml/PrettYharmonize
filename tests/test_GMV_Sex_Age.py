...
from juharmonize import JuHarmonize, JuHarmonizeCV
...
model_harm_cv = JuHarmonizeCV(problem_type=problem_type, 
                              pred_model=pred_model_JuHaCV,
                              n_folds=10,
		regression_points=100, stack_model=stack_model_JuHaCV,
		regression_search=False, regression_search_tol=float(2.0), 
...
print(f'Variance decreased for {np.sum(np.std(X_harm, axis=0) < np.std(X, axis=0))} features of {X.shape[1]}')
...
for i_fold, (train_index, test_index) in enumerate(kf.split(X)):
    ...
    X_train_harm_notarget = model_notarget.fit_transform(X_train, y_train, sites_train, covars_train)
    X_test_harm_notarget = model_notarget.transform(X_test, y_test, sites_test, covars_test)
    ...
    X_train_harm_cv = model_harm_cv.fit_transform(X_train, y_train, sites_train, 
                        covars_train)
    ...
    if problem_type == 'binary_classification':
        ...
        pred_notarget = pred_model_notarget.predict_proba(X_test_harm_notarget)[:,1]
        ...
        # noleak
        pred_noleak = pred_model_noleak.predict_proba(X_test_harm_cv)[:,1]
        ...
        # noleak
        pred_noleak = model_harm_cv._pred_y
